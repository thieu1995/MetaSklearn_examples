...
from data_util import get_cdc_diabetes
...
def suggest_rf_params(trial):
    return {
        ...
        "max_depth": trial.suggest_int("max_depth", 6, 12),  # exclude None to avoid dtype issue
        "min_samples_split": trial.suggest_int("min_samples_split", 2, 20)
    }

def suggest_mlp_params(trial):
    return {
        "hidden_layer_sizes": trial.suggest_categorical("hidden_layer_sizes", [(20, 5), (10, 5),]),
...
if __name__ == "__main__":
    ...
    HIDDEN_SET = [(50, 20), (80, 40), (100, 50)]
    ALPHA_SET = [0.001, 0.01, 0.1]
    BATCH_SIZE_SET = [512, 1024, 2048]
    ...
    param_spaces = {
        ...
        "RF": {
            "n_estimators": skace.Integer(50, 500),
            "max_depth": skace.Integer(6, 12),  # exclude None to avoid type mismatch in skopt
            "min_samples_split": skace.Integer(2, 20),
        },
        "MLP": {
            "hidden_layer_sizes": skace.Categorical([100, 70, 50]),  # HIDDEN_SET
    ...
    param_bounds = {
        ...
        "RF": [
            IntegerVar(lb=50, ub=500, name="n_estimators"),
            IntegerVar(lb=6, ub=12, name="max_depth"),  # exclude None to avoid type mismatch in skopt
            IntegerVar(lb=2, ub=20, name="min_samples_split"),
        ],
        "MLP": [
    ...
    ## Load data object
    X_train, X_test, y_train, y_test = get_cdc_diabetes()
