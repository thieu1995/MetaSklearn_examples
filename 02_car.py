...
from data_util import get_car_evaluation
...
def suggest_rf_params(trial):
    return {
        ...
        "max_depth": trial.suggest_int("max_depth", 3, 6),  # exclude None to avoid dtype issue
        "min_samples_split": trial.suggest_int("min_samples_split", 3, 7),
        "min_samples_leaf": trial.suggest_int("min_samples_leaf", 1, 3),
        "max_features": trial.suggest_categorical("max_features", ['sqrt', 'log2', 3, 4, 0.15, 0.25])
    }


def suggest_mlp_params(trial):
    return {
        "hidden_layer_sizes": trial.suggest_categorical("hidden_layer_sizes", [(20,), (25, ), (30, ), (15, )]),
...
if __name__ == "__main__":
    ## Configurations
    HIDDEN_SET = [(20,), (25, ), (30, ), (15, )]
    ALPHA_SET = [0.001, 0.01, 0.1, 0.2]
    BATCH_SIZE_SET = [8, 16, 32]
    ...
    param_grids = {
        ...
        "RF": {
            ...
            "max_features": ['sqrt', 'log2', 3, 4, 0.15, 0.25]
        },
        "MLP": {
    ...
    param_spaces = {
        "SVC": {
            "C": skace.Real(0.1, 10., prior='log-uniform'),  # log scale between 1 and 50
        ...
        "RF": {
            "n_estimators": skace.Integer(20, 60),
            "criterion": skace.Categorical(['gini', 'entropy']),
            "max_depth": skace.Integer(3, 6),  # exclude None to avoid type mismatch in skopt
            "min_samples_split": skace.Integer(3, 7),
            "min_samples_leaf": skace.Integer(1, 3),  # minimum leaf size
            "max_features": skace.Categorical(['sqrt', 'log2', 3, 4, 0.15, 0.25])  # categorical features
        },
        "MLP": {
    ...
    param_bounds = {
        ...
        "RF": [
            IntegerVar(lb=20, ub=60, name="n_estimators"),
            StringVar(valid_sets=('gini', 'entropy'), name="criterion"),
            CategoricalVar(valid_sets=(None, 3, 4, 5, 6), name="max_depth"),
            IntegerVar(lb=3, ub=7, name="min_samples_split"),
            IntegerVar(lb=1, ub=3, name="min_samples_leaf"),
            CategoricalVar(valid_sets=('sqrt', 'log2', 3, 4, 0.15, 0.25), name="max_features")
        ],
        "MLP": [
    ...
    ## Load data object
    X_train, X_test, y_train, y_test = get_car_evaluation()
    ...
    with ProcessPoolExecutor(max_workers=Config.N_WORKERS) as executor:
        ...
        for model in LIST_MODELS:
            pathsave = f"{Config.PATH_SAVE}/{Config.DATA_02}/{model['name']}_results.csv"
